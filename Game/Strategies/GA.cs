...
//		public GA(Config config, Random random)
//		{
//			this.config = config;
//			this.random = random;
//		}
...
//		public List<Creature> Simulate(Simulator sim, int player)
//		{
...
//			//        0.3...1.0 - move according to gene1 and gene2
//			//
...
//			//        0.40...1.00 - move according to gene1 and gene2
//			//
//			//   xGene (0...1) - x (0...config.GAME_WIDTH)
//			//   yGene (0...1) - y (0...config.GAME_HEIGHT)

//			var stopwatch = Stopwatch.StartNew();

//			Logger.Debug($"GLOBAL: {sim.globalTargets[sim.globalTargetIndices[player]].x} {sim.globalTargets[sim.globalTargetIndices[player]].y}");
//			Logger.Debug($"GLOBAL: {sim.globalTargets[(sim.globalTargetIndices[player] + 1) % sim.globalTargets.Length].x} {sim.globalTargets[(sim.globalTargetIndices[player] + 1) % sim.globalTargets.Length].y}");
//			Logger.Debug($"GLOBAL: {sim.globalTargets[(sim.globalTargetIndices[player] + 2) % sim.globalTargets.Length].x} {sim.globalTargets[(sim.globalTargetIndices[player] + 2) % sim.globalTargets.Length].y}");
//			var population = GeneratePopulation(sim, player);
//			var generation = 0;
//			//while (generation < 100)
//			while (stopwatch.ElapsedMilliseconds < 18)
//			{
//				var clone = sim.Clone();
//				var creature = EvolveNewCreature(population);
//				creature.desision = Simulate(clone, player, creature.genome);
//				creature.score = Evaluate(clone, player);
//				Logger.Debug($"AFTER: {creature.score} {string.Join(" ", creature.genome)}");
//				population.Add(creature);
//				population.Sort((g1, g2) => g1.score > g2.score ? -1 : g1.score < g2.score ? 1 : 0);
//				population.RemoveAt(population.Count - 1);
//				generation++;
//			}
//			Logger.Info($"Generation: {generation}");
//			Logger.Info($"Best score: {population[0].score}");
...
//		public Direct Simulate(Simulator sim, int player, double[] genome)
//		{
//			Direct result = null;
//			for (var i = 0; i < sim.players[player].Count; i++)
//				Logger.Debug($"BEFORE: {sim.players[player][i].x} {sim.players[player][i].y}");
...
//				for (var i = 0; i < sim.players[player].Count; i++)
//					Logger.Debug($"GROUP: {sim.players[player][i].x} {sim.players[player][i].y}");
//			}
//			return result;
//		}

//		public double Evaluate(Simulator sim, int player)
//		{
//			if (sim.players[player].Count == 0)
//				return double.MinValue;

//			var score = sim.scores[0] - sim.scores.Skip(1).Max();
//			var minGlobalQDist = sim.players[player].Min(p => p.QDistance(sim.globalTargets[sim.globalTargetIndices[player]]));
...
//			var result = 10000.0 * score
//			             + 10 * sim.globalTargetsTaken[player]
//			             + 0.5 * (config.GAME_HEIGHT - Math.Sqrt(minGlobalQDist)) / config.GAME_HEIGHT;
...
//		private void Simulate(Simulator sim, int player, double metaGene, double xGene, double yGene, out Direct desision)
//		{
//			var cache = new Cache();
//			if (HasEnemies(sim, player))
//			{
...
//						SimulateSplitToFoodOrTarget(sim, player, (int)((metaGene - ENEMY_AGGRESSOR_LIMIT) * GA_TURNS / (ENEMY_SPLIT_TO_TARGET_LIMIT - ENEMY_AGGRESSOR_LIMIT)), cache, out desision);
//					else
//						SimulateMoveToFoodOrTarget(sim, player, cache, out desision);
...
//				{
//					SimulateSplitToFoodOrTarget(sim, player, (int)(metaGene * GA_TURNS / NOENEMY_SPLIT_TO_TARGET_LIMIT), cache, out desision);
//					return;
//				}
...
//			SimulateMoveWithTargetPoint(sim, player, xGene, yGene, cache, out desision);
//		}

//		private void SimulateMoveWithTargetPoint(Simulator sim, int player, double xGene, double yGene, Cache cache, out Direct desision)
//		{
//			desision = null;

//			var x = xGene * config.GAME_WIDTH;
//			var y = yGene * config.GAME_HEIGHT;

//			var directs = new Direct[] { null, null, null, null };
//			directs[player] = desision = new Direct(x, y, config);
...
//		private void SimulateMoveToFoodOrTarget(Simulator sim, int player, Cache cache, out Direct desision)
//		{
//			var directs = new Direct[] { null, null, null, null };
//			desision = null;
//			for (var i = 0; i < GA_TURNS; i++)
...
//				directs[player] = new Direct(food ?? sim.globalTargets[sim.globalTargetIndices[player]], config);
//				if (desision == null)
//					desision = directs[player];
//				sim.Tick(directs);
//			}
...
//		private void SimulateSplitToFoodOrTarget(Simulator sim, int player, int turn, Cache cache, out Direct desision)
//		{
//			var directs = new Direct[] { null, null, null, null };
//			desision = null;
//			for (var i = 0; i < GA_TURNS; i++)
