...
namespace UnitTests
{
	...
	public unsafe class Simuator_Test
	{
		...
		public void METHOD2()
		{
			...
			var globalTargets = new[] { new Point(0, 0), new Point(0, 0), new Point(0, 0), new Point(0, 0) };
			var simulator = new Simulator(simState, globalTargets, new[] { 0, 0, 0, 0 });
			var directss = new[] { new Direct(0, 0, config), new Direct(0, 0, config), new Direct(0, 0, config), new Direct(0, 0, config) };
			simulator.Tick(directss);
			simulator.Tick(directss);
			simulator.Tick(directss);
			simulator.Tick(directss);
			...
			for (var i = 0; i < simulator.players.Length; i++)
			{
				Console.Out.WriteLine($"Player {i}");
				var fragments = simulator.players[i];
				foreach (var frag in fragments)
					Console.Out.WriteLine(frag);
